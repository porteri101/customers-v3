...
(function() {
  ...
  angular.module('cui.base', ['cui.templates', 'smart-table', 'pascalprecht.translate', 'keyboard', 'dateParser', 'ui.select', 'ngSanitize']).value('baseTemplatePath', '__cui/').config(function($translateProvider) {
    $translateProvider.translations('en', {
      ...
      "CUI_ALERT_DANGER": "Critical Error",
      "CUI_ALERT_WARNING": "Warning",
      "CUI_ALERT_UNKNOWN": "There are multiple issues",
      "CUI_ALERT_SUCCESS": "Success",
      "CUI_ALERT_NOTE": "Note"
    });
  ...
  /**
    ...
    @module cui.charts
    @description Charts allow you to present data in a visual and more consumable manner. CUI Charts implements two 3rd party libraries.
  
  <h3>Chart Libraries</h3>
  
  [Chart.js](http://www.chartjs.org/) provides the foundation for the charts. It is a highly extendable and configurable charting library providing access to API's allowing the flexibility to configure pre-built charts or create new chart types.
  
  [Angular-Chart.js](http://jtblin.github.io/angular-chart.js/) is an AngularJs wrapper for the Chart.js library. It provides high-level access to the Chart.js library without blocking access to the core Chart.js API.
  
  The addition of these libraries has a minimal impact on the overall file size of CUI.
  
  To include charts in your application, you will need to include
  ```html
  <script src="/path/to/cui/js/cui-charts.min.js"></script>
  ```
  in the `<head>` of your HTML after the `vendor.min.js` include. Then in your controller add `cui.charts` as a dependency
  ```js
  angular.module('app', ['cui', 'cui.charts']);
  ```
    ...
    @param {string} chartType The type chart. This is an attribute only. One of:
      ** `chart-line`
      ** `chart-pie`
      ** `chart-doughnut`
      ** `chart-bar`
      ** `chart-polar`
      ** `chart-radar`
    @param {array} data Series data for the chart to render.
    @param {array} labels Display labels for the chart; location of labels vary by chart type.
    @param {boolean=} legend Display a legend below the chart; defaults to false.
    @param {array=} series Display data series labels. Default is `[]`. Does not apply to doughnut, pie, and polar area chart types.
    @param {expression=} click `onclick` event handler. Does not apply to doughnut, pie, and polar area chart types.
    @param {array=} options Customization options specific to an individual chart.
    @param {array=} colours The color palette an individual chart should render using. The spelling is proper for the dependencies.
  
    @example
    <h3>Line Chart- Single Data Set</h3>
  <example name='line-single'>
    <file name='index.html'>
      <div>
        <canvas chart-line class='chart chart' data='data'
          labels='labels' legend='true' series='series'
          click='onClick'></canvas>
      </div>
    </file>
    <file name='app.js'>
      var module = angular.module('app', ['cui', 'cui.charts']);
      module.controller('AppCtrl', function($scope, $q, DataFactory) {
          var data = DataFactory.get();
  
          $scope.series = ['Atlas'];
          $scope.labels = data.labels;
          $scope.data = data.points;
    ...
    <file name='data.factory.js'>
      module.factory('DataFactory', function() {
        // A contrived example simulating the retrieval of data from a factory
  ...
  @example
    <h3>Line Chart - Multiple Data Sets</h3>
  <example name='line-multiple'>
    <file name='index.html'>
      <div>
        <canvas chart-line class='chart' data='data'
          labels='labels' legend='true' series='series'
          click='onClick'></canvas>
      </div>
    </file>
    <file name='app.js'>
      var module = angular.module('app', ['cui', 'cui.charts']);
      module.controller('AppCtrl', function($scope, DataFactory) {
        var data = DataFactory.get();
  
        $scope.series = ['Atlas', 'CMS', 'Alice', 'LHCb'];
        $scope.labels = data.labels;
        $scope.data = data.points;
        ...
        // A contrived example simulating the retrieval of data from a factory
          ...
          var getPoints = function() {
            return [[65, 59, 80, 81, 56, 55, 40],
                      ...
                      [57, 69, 75, 68, 77, 65, 42],
                      [22, 33, 45, 12, 43, 20, 50]];
          };
  ...
  <example name='bar'>
    ...
    <file name='index.html'>
      <canvas chart-bar class="chart" data="data" labels="labels"></canvas>
    </file>
  
    <file name='app.js'>
      angular.module('app', ['cui', 'cui.charts']).controller('AppCtrl', function($scope) {
        $scope.labels = ['Atlas', 'CMS', 'Alice', 'LHCb']
  ...
  <example name='doughnut'>
  
    <file name='index.html'>
      <canvas id='doughnut' chart-doughnut class="chart" data="data" labels="labels" legend="true"></canvas>
    </file>
  
    <file name='app.js'>
      angular.module('app', ['cui', 'cui.charts']).controller('AppCtrl', function($scope) {
        $scope.labels = ['Atlas', 'CMS', 'Alice', 'LHCb'];
        $scope.data = [300, 500, 100, 250];
      })
    </file>
  </example>
  ...
  <example name='pie'>
    ...
    <file name='index.html'>
      <canvas id='pie' chart-pie class="chart" data="data" labels="labels" legend="true"></canvas>
    </file>
  ...
  <example name='radar'>
    ...
    <file name='index.html'>
      <canvas id="radar" chart-radar class="chart" data="data" labels="labels"></canvas>
    </file>
  ...
  <example name='polar-area'>
    <file name='index.html'>
      <canvas id="polar-area" chart-polar-area class="chart" legend='true' data="data" labels="labels"></canvas>
    </file>
    <file name='app.js'>
      angular.module('app', ['cui', 'cui.charts']).controller('AppCtrl', function($scope) {
        $scope.labels = ["Download Sales", "In-Store Sales", "Mail-Order Sales", "Tele Sales", "Corporate Sales", "additional", "External"];
        $scope.data = [300, 500, 100, 140, 220, 150, 200];
      })
    </file>
  </example>
    ...
    @example
      <h3>Dynamic Chart (Chart Base)</h3>
  <example name='chart-base'>
  
    <file name='index.html'>
      <canvas id="base" class="chart-base" chart-type="type" data="data" labels="labels" legend="true"></canvas>
      <cui-button cui-type="primary" ng-click="toggle()">Toggle Chart Type</cui-button>
    </file>
    ...
    @example
      <h3>Chart Colors</h3>
  <p>Colors are useful for helping your users to comprehend the data they are viewing. While there are no 'right' colors, their are definitely 'wrong' colors.</p>
  
  <p>CUI Charts color palette is based on the Dell brand colors that have been lightened and desaturated.</p>
  <p>Default chart colors</br>
  Blue: #2e87b2 <br/>
  Gray: #aaaaaa<br/>
  Violet: #895997<br/>
  Red: #cc4755<br/>
  ...
  <p>You can override the default colors in each individual chart based on how you want to call attention to the data.
  
  Blues & Grays: Neutral colors that allow the user to focus on the data as a whole.</br>
  Violet: Multi-purpose; can be used to add a color variant to the blues and grays.</br>
  Red: Draw attention to data that may be a cause for concern.</br>
  Orange: Draw attention to data that may be a cause for concern.</br>
  Yellow: Draw attention to data that may be a cause for concern.</br>
  </p>
  
  <p>While colors usually have a significant meaning, they may still be used together to signify the difference in data, or to have clear contrast amongst data sets. Red, orange, yellow do not always need to be used as alerting colors.</p>
  
  <example name='chart-colors'>
    <file name='index.html'>
      <div style='width: 40%'>
        <h3>CPU Usage</h3>
        <canvas chart-line class='chart' options='lineChart.options' data='lineChart.data' labels='lineChart.labels' series='lineChart.series' legend='true'></canvas>
      </div>
  
      <div style='width: 40%'>
        <canvas chart-bar class='chart' colours='barChart.colors' data='barChart.data' labels='barChart.labels'></canvas>
      </div>
    </file>
    ...
    <file name='app.js'>
      angular.module('app', ['cui', 'cui.charts']).controller('AppCtrl', function($scope) {
        $scope.lineChart = {
          ...
          series: ['Atlas', 'CMS', 'Alice', 'LHCb'],
          data: [[30, 20, 30, 22, 30, 25, 33],
                  [22, 25, 33, 35, 22, 25, 30],
                  [25, 60, 60, 60, 35, 30, 20],
                  [35, 40, 33, 22, 22, 25, 33]],
          options: {
            datasetFill: false
          }
        }
        ...
        $scope.barChart = {
          data:  [[25], [60], [60], [60], [35], [30], [20]],
          labels: ['Alice High Usage'],
          colors: ['#84b323', '#cc4755', '#cc4755', '#cc4755', '#84b323', '#84b323', '#84b323']
        }
      })
    </file>
  </example>
  ...
  module = angular.module('cui.charts', ['chart.js']);
  ...
  module.config(function(ChartJsProvider) {
    ...
    return ChartJsProvider.setOptions({
      colours: ['#2e87b2', '#aaaaaa', '#895997', '#cc4755', '#ed8d54', '#efd38b', '#84b323'],
      responsive: true
    });

    /*
     * Individual chart type configuration
     */
  });
  ...
  ** Other: `'ok-sign'`
  ...
  module.directive('cuiAlert', function(baseTemplatePath, $timeout, $q, $translate) {
    var TRANSITION_TIME, getDefaults;
    TRANSITION_TIME = 250;
    getDefaults = function(type) {
      switch (type) {
        case 'danger':
          return ['exclamation-sign', $translate.instant('CUI_ALERT_DANGER')];
        case 'warning':
          return ['warning-sign', $translate.instant('CUI_ALERT_WARNING')];
        case 'unknown':
          return [null, $translate.instant('CUI_ALERT_UNKNOWN')];
        case 'success':
          return ['ok-sign', $translate.instant('CUI_ALERT_SUCCESS')];
        default:
          return ['info-sign', $translate.instant('CUI_ALERT_NOTE')];
      }
    };
    return {
  ...
  <example name='buttonTransparent'>
    ...
    <file name='style.css'>
      .docs-block { padding: 30px; }
      .blue { background-color: #007db8; }
  ...
  module.directive('cuiButton', function(baseTemplatePath) {
    return {
      templateUrl: "" + baseTemplatePath + "button.html",
      restrict: 'EA',
      transclude: true,
      replace: true,
      scope: {
      ...
      compile: function(element, attrs) {
        ...
        element.addClass("cux-button-" + attrs.size);
        ...
        return function(scope, element) {
          ...
          if (scope.cuiType != null) {
            element.addClass("cux-button-" + scope.cuiType);
          }
          return scope.$watch('cuiType', function(newClass, oldClass) {
            if (oldClass !== newClass) {
              element.removeClass("cux-button-" + oldClass);
              if (newClass != null) {
                return element.addClass("cux-button-" + newClass);
              }
            }
          });
        };
  ...
  <blockquote class='danger'>There is a known issue with IE9 whereas the browser does not repaint until an event occurs. This means the example may show an incorrect model in the rendered view, however the model is correct. You can test this by clicking the 'Log Models' button and viewing the models in the console.</blockquote>
  ...
  <example name='basicCheckbox'>
    <file name='index.html'>
      ...
      <p><cui-button ng-click='logModel()'>Log Model</cui-button></p>
    </file>
    <file name='app.js'>
      angular.module('app', ['cui'])
        .controller('AppCtrl', function($scope) {
  ...
  <example name='formCheckbox'>
    ...
    <file name='app.js'>
      angular.module('app', ['cui'])
        .controller('AppCtrl', function($scope) {
          $scope.logModel = function() {
            console.log('formData.movie', $scope.formData.movie);
          }
        });
    </file>
  </example>
  ...
  <example name='formCustomCheckbox'>
    ...
    <file name='app.js'>
      angular.module('app', ['cui'])
        .controller('AppCtrl', function($scope) {
          $scope.logModel = function() {
            console.log('formData.approve', $scope.formData.approve)
          }
        });
    </file>
  </example>
      ...
      <form novalidate name='checkboxExampleForm' class='cui-form-stacked'>
        <div class='cui-form-group'>
          ...
          <cui-checkbox-group name='colors' check-items='checkboxExample.colors'
                              ...
                              check-label='color'
                              ng-disabled='isDisabled'
                              ng-required='isRequired'></cui-checkbox-group>
        ...
        <p><cui-button ng-click='toggleDisabled()'>{{isDisabled && 'Enable' || 'Disable'}} checks</cui-button></
        p>
        <p><cui-button ng-click='logModel()'>Log Models</cui-button></p>
      </form>
    </file>
    <file name='app.js'>
      angular.module('app', ['cui'])
        .controller('AppCtrl', function($scope) {
          ...
          $scope.logModel = function() {
            console.log('formData.colors', $scope.formData.colors);
            console.log('formData.othercolors', $scope.formData.othercolors);
            console.log('formData.roles', $scope.formData.roles);
            console.log('formData.tasks', $scope.formData.tasks);
          }
    ...
    <file name='index.html'>
      <cui-drop-down-button label='Drop down button' cui-type='primary' icon="dashboard">
        <ul cui-keyboard-menu>
          <li ng-repeat='item in $parent.items' ng-click='$parent.$parent.click("Example 1 - {{item}}")'>{{item}}</li>
        </ul>
      ...
      compile: function(element, attrs) {
        ...
        return function(scope, element, attrs, ctrl, transclude) {
          ...
          return transclude(scope, function(content) {
            ...
            contents = $compile(ul)(tipScope)[0];
  ...
  ```
    ...
    @deprecated 2.8.0
    @useInstead We're discussing the future of form elements in CUI. Use the class `.cui-drop-down-list` directly, or use <a href='../uiSelect'>UI Select</a> instead for more complex applications.
  ...
  module.factory('cuiIcon', function() {
    ...
    return {
      ...
      getIconClass: function(icon) {
        ...
        return "cui-icon-" + icon + " cux-icon-" + icon;
      }
    };
  });
  ...
  <example name='masthead'>
    <file name='index.html'>
      <cui-masthead application-name='Active System Manager' application-subname='Admin panel' show-controls=true application-logo="http://plnkr.co/img/plunker.png">
        <!-- Masthead controls -->
        <cui-drop-down-button cui-type="transparent" label='Drop down button' icon="dashboard">
          <ul cui-keyboard-menu>
        ...
        <cui-button cui-type="transparent" ng-click='showSettings()'>
          <cui-icon icon='cog'></cui-icon> Settings
        </cui-button>
  
        <cui-button cui-type="transparent" ng-click='showAboutBox()'>
  ...
  /**
    ...
    @deprecated 2.8.0
    @useInstead Please consider using <a href='https://code.angularjs.org/1.3.10/docs/api/ngMessages/directive/ngMessages'>NG Messages</a> instead.
  ...
  module.directive('cuiMemo', function(baseTemplatePath, $compile) {
    return {
      ...
      link: function(scope, iElement, iAttrs, ngModelCtrl) {
        var form, memoElement, memos, parent;
        ...
        ngModelCtrl.$validators['cuiMemo'] = function(modelValue, viewValue) {
          ...
          if (ngModelCtrl.$dirty) {
            return memoElement.text(memo != null ? memo : '');
          }
        };
    ...
    return {
      ...
      controller: function($scope, $location) {
        ...
        this.addNode = function(node) {
          nodes.push(node);
          if ($scope.watchLocation) {
            if ((node.items.href != null) && (decodeURIComponent($location.absUrl())).indexOf(node.items.href) !== -1) {
              selectNode(node);
            }
          }
        };
  ...
  <example name='indeterminateProgressBar'>
    ...
    <p><b>Known Issue:</b> The stripes will not be displayed in IE 9. The bar will still be partly transparent.</p>
          ...
          (function loop() {
            ...
            $timeout(function() {
              if ($scope.value > 100) {
                ...
                var newValue = $scope.value + Math.round(Math.random() * 25) + 5;
                $scope.value = newValue < 100 ? newValue : 100;
              }
  ...
  <example name='basicRadio'>
    ...
    <file name='app.js'>
      angular.module('app', ['cui'])
        .controller('AppCtrl', function($scope) {
          $scope.logModel = function() {
            console.log('orchestra', $scope.orchestra)
          }
        });
    </file>
  </example>
  ...
  <example name='individualRadio'>
    <file name='index.html'>
      <form novalidate name='radioExampleForm' class='cui-form-stacked'>
        ...
        <cui-button ng-click='logModel()'>Log Model</cui-button>
      </form>
    </file>
    <file name='app.js'>
      angular.module('app', ['cui'])
    ...
    <file name='index.html'>
      <form novalidate name='radioExampleForm' class='cui-form-stacked'>
        <div class='cui-form-group'>
          ...
          <code>formData.roles: {{formData.roles}}</code> <br />
          <cui-button ng-click='logModel()'>Log Model</cui-button>
        </div>
      </form>
    </file>
  ...
  This version of the editor allows for easier configuration and sets up the API to allow for user defined tools, which is a feature that is on the roadmap. We've added a color palette for font color and background color, cleaned up the API considerably and have laid the groundwork to allow extraction of the HTML content and, in the future, the ability to extract plain text while preserving line breaks for block level elements.
  
  <blockquote class='danger'>Rich Text Editor is not supported in IE9 die to browser API inconsistencies. We recommend using a 3rd party rich text editor such as <a href='http://textangular.com/' target='_blank'>textAngular</a>.</blockquote>
  
    @deprecated 2.8.0
    @useInstead Use a 3rd party rich text editor such as <a href='http://textangular.com/' target='_blank'>textAngular</a>.
  ...
  
    {
      ...
      tools: { # leave empty or set individual tools to false if you do now want to provide them
        bold:  ,
        italic: ,
        underline: ,
        strikethrough: ,
      ...
      angular.module('app', ['cui'])
        .controller('AppCtrl', function($scope) {
          ...
          $scope.getContent = function() {
            ...
            console.log('Button Content As Text:', $scope.settings.content)
            console.log('--------------------------------------------')
          }
        })
    </file>
  ...
  <example name='table'>
    ...
    <blockquote class='danger'>This example takes advantage of CSS FlexBox properties which has limited support in Internet Explorer 9 and 10. Your mileage may vary.</blockquote>
  <example name='table'>
    <file name='index.html'>
  ...
  <example name='formTextarea'>
    ...
    <p><b>Known Issue:</b> IE 9 will not change the cursor to indicate the input has been disabled.</p>
   */
  ...
  ***For most use cases, please see the Time Picker, which implements this inside of a tip.***
  ...
  > The Dell logo is provided by default, however Clarity 4 strongly recommends using a custom logo for your product. If you
  > need a logo, the [Clarity team](mailto:uxdesign@dell.com) may be able to help.
  ...
  `applicationName` is also used as the `persistAs` value for the Navigation List.
    ...
    @example
    Content inside a `cuiApplicationFrame` will be wrapped with a Masthead and (optionally) a navigation sidebar.
  ```
  ...
  ```
    ...
    @example
    The `cuiApplicationFrame` also supports complex/multi transclusion, allowing controls to be added to the right side of the Masthead.
  ```
  <cui-application-frame ... >
    <div cui-controls>
      <!-- Controls go here -->
  ...
  <example name="basicApplicationFrame">
    <file name="index.html">
      <cui-application-frame application-name="The Base for your App" application-logo="http://plnkr.co/img/plunker.png">
        <h2>Hello, world!</h2>
        <cui-button cui-type="primary">
  ...
  <example name="basicApplicationFrame">
    <file name="index.html">
      <cui-application-frame application-name="Example title" application-href="http://www.dell.com"
        application-subname="SubTitle" masthead-show-controls=true navigation-items="navigationItems">
        <div cui-controls>
          <cui-drop-down-button label="Drop Down Button" cui-type="primary" icon="dashboard">
            <ul cui-keyboard-menu>
              <li kb-focus="focus-1" ng-click="item('Example 2 - Item 1')">Item 1</li>
              <li kb-focus="focus-2" ng-click="item('Example 2 - Item 2')">Item 2</li>
              <li kb-focus="focus-3" ng-click="item('Example 2 - Item 3')">Item 3</li>
            </ul>
          </cui-drop-down-button>
  ...
  module.directive('cuiApplicationFrame', function(baseTemplatePath, $document, $timeout) {
    return {
      ...
      link: function(scope, element) {
        var drawerButton, handleClick, handleResize;
        scope.drawerShowing = false;
        scope.$watch('drawerShowing', function(drawerShowing) {
          if (drawerShowing === true) {
            return angular.element(document.body).addClass('cux-masthead-navigation-open');
          } else if (drawerShowing === false) {
            return angular.element(document.body).removeClass('cux-masthead-navigation-open');
          }
        });
        handleClick = function() {
          scope.drawerShowing = false;
        ...
        scope.toggleDrawer = function() {
          angular.element(document.body).removeClass('cux-navigation-open');
          scope.drawerShowing = !scope.drawerShowing;
          $timeout(function() {
            if (scope.drawerShowing) {
              $document.bind('click', handleClick);
        ...
        scope.$on('cui:masthead:controls:close', function() {
          return scope.drawerShowing = false;
        });
        drawerButton = angular.element(element[0].querySelector('.cui-application-frame-drawer'));
        scope.persistAs = scope.applicationName;
        if (scope.navigationPersist === false) {
          scope.persistAs = '';
        }
        scope.toggleNavigation = function() {
          return angular.element(document.body).toggleClass('cux-navigation-open');
        };
        handleResize = scope.handleResize = function() {
          return angular.element(document.body).removeClass('cux-navigation-open');
        };
        ...
        this.goto = (function(_this) {
          return function(identifier) {
            ...
            if (angular.isString(identifier)) {
              for (i = _i = 0, _len = steps.length; _i < _len; i = ++_i) {
                step = steps[i];
                if (step.cuiTitle === identifier || step.title === identifier) {
  ...
  <example name="okcanceldialog">
    <file name="index.html">
      <cui-button ng-click="prompt()">Show Dialog</cui-button>
  ...
  <example name="modal">
    ...
    <file name="modal.html">
      <header><h1 class="cui-title">My ({{name}}) first modal</h1></header>
...
$templateCache.put("__cui/button.html","<button class=\"cui-button cux-button\" ng-transclude></button>");
...
$templateCache.put("__cui/dropDownButton.html","<div class=cui-drop-down-button><cui-button><span ng-if=icon><cui-icon class=cui-drop-down-button-icon icon={{icon}}></cui-icon>&#32;</span>{{label}}&#32;<cui-icon icon=caret-down></cui-icon></cui-button></div>");
$templateCache.put("__cui/dropDownList.html","<select class=cui-drop-down-list ng-transclude></select>");
$templateCache.put("__cui/icon.html","<i class=\"cui-icon cui-icon-color-{{color}} cux-icon\" ng-style=\"{&quot;font-size&quot;: size}\"></i>");
$templateCache.put("__cui/masthead.html","<header class=cui-masthead ng-class=\"{fixed: fixed}\"><h1 class=cui-application-title><a ng-href={{applicationHref}} class=cui-application-title-link ng-if=\"!applicationLogo && applicationHref\"><span class=cui-application-name>{{name}}</span></a> <span class=cui-application-name ng-if=\"!applicationLogo && !applicationHref\">{{name}}</span> <a ng-href={{applicationHref}} class=cui-application-title-link ng-if=\"applicationLogo && applicationHref\"><span class=cui-application-name><img class=cui-application-logo ng-src={{applicationLogo}}></span></a> <span class=cui-application-name ng-if=\"applicationLogo && !applicationHref\"><img class=cui-application-logo ng-src={{applicationLogo}}></span> <span class=cui-application-subname ng-if=subname>{{subname}}</span></h1><span ng-show=showControls><div class=cui-hidden-lg><span class=cui-navigation-drawer ng-show=showNavigationDrawer ng-click=$parent.toggleNavigation()><cui-icon icon=angle-right></cui-icon></span> <span class=cui-drawer ng-class=\"{&quot;cui-drawer-showing&quot;: drawerShowing}\" ng-show=showControls ng-click=toggleDrawer()><cui-icon icon=ellipsis-vertical></cui-icon></span></div><div ng-transclude class=\"cui-masthead-controls horizontal-list\" ng-class=\"{&quot;cui-masthead-drawer-showing&quot;: drawerShowing}\"></div></span></header>");
...
$templateCache.put("__cui/progressBar.html","<div class=cui-progress-bar><progress max=100 title=\"{{value || \'Unknown\'}} percent complete\" ng-attr-value={{value}} class=cui-progress-bar-type-{{cuiType}}><div class=cui-progress-bar-fallback><span ng-style=\"{\'width\': value + \'%\'}\">{{value || \'Unknown\'}} percent complete</span></div></progress></div>");
...
$templateCache.put("__cui/tabset.html","<div class=\"cui-tabset cux-tabset\"><ul class=\"cui-tabs cui-horizontal-list cux-tabs\"><li class=\"cui-tab cux-tab\" ng-repeat=\"tab in tabs\"><button class=cux-tab-button ng-class=\"{ \'cux-active\': tab.active }\" ng-click=tab.showTab()><cui-icon ng-show=tab.icon icon={{tab.icon}}></cui-icon><span class=cux-tab-button-label>{{tab.label}}</span><cui-badge ng-show=tab.badge cui-type={{tab.badge.type}}>{{tab.badge.label}}</cui-badge></button></li></ul><div class=\"cui-tab-container cux-tab-content\" ng-transclude></div></div>");
...
$templateCache.put("__cui/aboutBox.about.html","<div class=cui-about-pane><div class=cui-about-pane-center><cui-icon class=cui-about-logo color=blue icon=dell-halo></cui-icon><div class=cui-title-group><h1 class=cui-title>{{modal.applicationName}}</h1><span class=cui-version>{{modal.version}}</span></div><div class=cui-product-info><p ng-if=modal.patentString>{{modal.patentString}}</p><p translate=CUI_ABOUTBOX_DELL_TRADEMARK></p><p>&copy; {{today | date:\"yyyy\"}} Dell Inc. ALL RIGHTS RESERVED.</p></div></div></div>");
$templateCache.put("__cui/aboutBox.contact.html","<div class=cui-contact-pane><h1 class=cui-title translate=CUI_ABOUTBOX_CONTACT_TITLE></h1><p translate=CUI_ABOUTBOX_CONTACT_TEXT></p><h2 translate=CUI_ABOUTBOX_TECHNICAL_TITLE></h2><p translate=CUI_ABOUTBOX_TECHNICAL_LINK></p><h2 translate=CUI_ABOUTBOX_PRODUCT_TITLE></h2><p translate=CUI_ABOUTBOX_PRODUCT_PHONE></p><h2 translate=CUI_ABOUTBOX_EMAIL_TITLE></h2><p translate=CUI_ABOUTBOX_EMAIL_LINK></p></div>");
...
$templateCache.put("__cui/applicationFrame.html","<div class=cui-application-frame><header class=\"cux-masthead cux-fixed-masthead\"><i class=\"cux-masthead-expand cux-icon cux-icon-angle-right\" ng-if=\"navigationItems !== undefined\" ng-click=toggleNavigation()></i><cui-icon ng-if=!applicationLogo class=cux-masthead-logo icon=dell-halo></cui-icon><img ng-src={{applicationLogo}} ng-if=applicationLogo class=cux-masthead-logo><h1 class=cux-masthead-title><a ng-if=applicationHref ng-href={{applicationHref}}>{{applicationName}}</a> <span ng-if=!applicationHref>{{applicationName}}</span></h1><h2 class=cux-masthead-subtitle>{{applicationSubname}}</h2><nav class=cux-masthead-controls cui-application-frame-transclude=cui-controls></nav><i class=\"cux-masthead-navigation-toggle cux-icon cux-icon-ellipsis-vertical cui-application-frame-drawer\" ng-if=mastheadShowControls ng-click=toggleDrawer()></i></header><nav class=\"cui-application-frame-navigation cux-fixed-side-navigation\" ng-if=\"navigationItems !== undefined\"><cui-navigation-list items=navigationItems persist-as={{persistAs}} watch-location=true></cui-navigation-list></nav><main class=\"cui-container cux-fixed-content\"><div id=cui-application-frame-alert-service></div><div class=\"cui-content cux-content-padding\" cui-application-frame-transclude=cui-content></div></main><div class=cux-fixed-backdrop ng-if=\"navigationItems !== undefined\" ng-click=toggleNavigation()></div></div>");
